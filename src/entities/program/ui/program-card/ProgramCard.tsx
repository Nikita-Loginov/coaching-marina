      ...
      <div className={scss["program__content"]}>
        ...
        <div className={scss["program__block"]}>
          ...
          <h2 className={classNames("h2", scss["program__title"])}>
            {wordsWithoutLastWord} <br /> <span className={scss["program__title-sub"]}>{lastWord}</span>
          </h2>

          <div
            className={classNames(scss["textbox"], scss["program__textbox"])}
          >
            {description.map((text, index) => {
              return <p className="p1" key={index}>{text}</p>;
            })}
          </div>
        </div>
